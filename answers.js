...
function isItGoingToRain (data1) {
    return data1.hourly.data.some(function (hourlyObj) {
        return hourlyObj.icon.toLowerCase().indexOf("rain") !== -1;
    });
}


console.log("is it going to rain?: " + isItGoingToRain(forecastData));
...
function timeBeforeEightHr(weatherData) {

    if (weatherData.time <= Math.floor((Date.now() / 1000)) + 28800) {
        return true;
    }

}

function isItRaining(data2) {
    return data2.icon.toLowerCase().indexOf("rain") !== -1;
}


console.log("Is it going to rain in the next 8 hours? " + forecastData.hourly.data.filter(timeBeforeEightHr).some(
        function (hourlyObj) {
            return hourlyObj.icon.toLowerCase().indexOf("rain") !== -1;
        }
    )
...
// console.log(forecastData.hourly.data.length);
// console.log(nextEightHr.length);
...
// methods(function function_name(argument) {
//     // body...
// }).method2(function function_name(argument) {
//     // body...
// })

// Exercise 3

var temps = forecastData.hourly.data.map(function(temp) {return temp.temperature});

console.log("The temperatures for this array are: " + temps);
...
console.log("Array of Date objects when it is raining: ", timeWhenItWillRain);

// Exercise 5

function isItSunnyErrDay (summValue) {
    return summValue.daily.data.every(function(hourlyObj) {
        return hourlyObj.summary.toLowerCase().indexOf("sun") !== -1;
    });
}

console.log("Will it be sunny every day? " + isItSunnyErrDay(forecastData));

// Exercise 6

var hottestTimeOfDay = forecastData.hourly.data.reduce( function(acc, temp) {
    
    if (temp.temperature > acc) {
...
console.log("This hottest temperature of the day = " + hottestTimeOfDay);



// Exercise 7

var weatherTypeTally = forecastData.daily.data.reduce( function (acc, weatherType) {
    
    if (acc[weatherType.icon]) {
        acc[weatherType.icon] = acc[weatherType.icon] + 1;
    }
    
    else {
        acc[weatherType.icon] = 1;
    }
    
    return acc;
}, {});

console.log("Weather Stats: ", weatherTypeTally);


// Exercise 8

var exerciseEightData = [
  {
    "id": 1,
    "name": "Leanne Graham",
    "username": "Bret",
...
function bizEmail (objInArr) {
    if (objInArr.email.toLowerCase().indexOf(".biz") !== -1) {
        return true;
    }
}

// extract only cities where they live
...
function uniqueCities (acc, city) {

        if (acc.indexOf(city) < 0) {
            acc.push(city);
        }
...
// put it together using 4 chained array methods :) 

exerciseEightData
.filter(bizEmail)
.map(cities)
.reduce(uniqueCities, [])
.forEach(printAnswers);
